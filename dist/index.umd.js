(function (global, factory) {
  ...
  (global = global || self, global.iframeMsg = factory());
  ...
  function getDomFromName(name) {
    ...
    if (typeof name === "string") {
      if (name[0] === "#" || name[0] === ".") return document.querySelector(name);else return window.frames[name];
    }
  ...
  function sendSon(obj, name) {
    return new Promise(function (resolve, reject) {
      ...
      if (!fr) {
        reject("\u6CA1\u6709name\u4E3A".concat(name, "\u7684\u5B50iframe"));
        console.warn("\u6CA1\u6709name\u4E3A".concat(name, "\u7684\u5B50iframe"));
        return;
      }

      var msgId = getRadId();
      fr.contentWindow.postMessage(JSON.stringify((_JSON$stringify = {}, _defineProperty(_JSON$stringify, K_ID, msgId), _defineProperty(_JSON$stringify, "data", obj), _JSON$stringify)), "*");
      promArrMapSon[msgId] = {
        resolve: resolve,
        reject: reject
      }; // 超时清理
